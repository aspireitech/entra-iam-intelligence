...
function ToxicCombinationsPage({data,collectorStatus}){
  ...
  return <div className="source-page">
    <Kpis items={[['Active Toxic Combinations',active.length,'☠']]}/>
    <section className="grid top-grid single"><Card title="Privileged Accounts With a Compounding Risk Signal">{!active.length?<div className="empty-state">No privileged account currently combines a second risk signal (missing MFA, ID Protection risk, or 90+ day staleness).</div>:<><FilterBar q={q} onQ={setQ} placeholder="Search name or signal…" count={filtered.length} total={active.length} exportRows={filtered} exportColumns={[{label:'User',value:'name'},{label:'Compounding Signals',value:c=>c.flags.join(', ')}]} exportFilename="toxic-combinations"/><div className="license-table"><table><thead><tr><th>User</th><th>Compounding Signals</th><th></th></tr></thead><tbody>{filtered.slice(0,300).map(c=><tr key={c.id}><td>{c.name}</td><td>{c.flags.join(', ')}</td><td><button className="attention-ack" title="Acknowledge with exception note" onClick={()=>acknowledge(c.id,c.name)}>✓</button></td></tr>)}</tbody></table></div></>}<div className="disclaimer">Privileged = assigned a detected built-in Administrator/Global Reader/Security Reader/Privileged Role Administrator role. A user needs privileged access AND at least one other live signal to appear here - never a guess. Large numbers here reflect real privileged-role assignment sprawl, not a rendering artifact - each row is a live cross-referenced finding.</div></Card></section>
...
function LegacyAuthenticationPage({data}){
  ...
  const sample=(la.sample||[]).filter(s=>!q||`${s.user} ${s.app} ${s.clientAppUsed}`.toLowerCase().includes(q.toLowerCase()));
...
function NonHumanIdentitiesPage({data}){
  ...
  return <div className="source-page">
    <Kpis items={[['Service Principals','totalServicePrincipals','⚶'],['Managed Identities','managedIdentities','◆'],['App Registrations','appRegistrations','▦'],['Credential-Bearing','credentialBearing','⏱',()=>goTo(setFocus,'nhi-credentials')],['Ownerless Apps','ownerlessCount','⚠',()=>goTo(setFocus,'ownerless-apps')]].map(([t,k,g,onClick])=>[t,nhi[k],g,onClick])}/>
    <SectionStack activeId={focus} sections={sections}/>
  </div>;
}
