...
function AzureSubscriptionsPage({data,azure,loading,onGrant}){
  ...
  if(!azure?.available){
    return <div className="source-page"><div className="permission-banner"><div><strong>Azure subscription access is not connected.</strong><span>Requires a separate consent (Azure Resource Manager, not Microsoft Graph) plus an Azure RBAC role - at least Reader - assigned to your account on the subscription(s) you want visible, from Azure Portal → Subscriptions → Access control (IAM). Being a Global Administrator in Entra ID does not grant that by itself.</span></div><button className="primary" onClick={onGrant}>{loading?'Requesting…':'Grant Azure permissions'}</button></div>{azure&&<div className="empty-state large">{loading?'Loading Azure subscription data…':`Unavailable: ${azure.reason||'Could not reach Azure Resource Manager.'}${azure.reason?.includes('Sign in')?'':' If you already granted permissions above, this usually means no Azure RBAC role is assigned to your account yet - ask an Azure subscription Owner to assign you at least Reader, then retry.'}`}</div>}</div>;
  }
  ...
  // of an extra Graph call. A principal not found here (most often a service
  // principal that never showed up as Entra-privileged) still renders correctly -
  // just as its raw ID with a type badge instead of a friendly name.
  ...
  const assignments=(azure.roleAssignments||[]).map(ra=>({...ra,principalName:nameById.get(ra.principalId)||ra.principalId}));
